<aside id="sidebar" class="bg-primary text-white w-64 hidden md:flex flex-col transition-all duration-300 z-20">
    ...
    <nav class="flex-1 px-2 py-4 space-y-2 overflow-y-auto styled-scrollbar">
        ...
        <a href="#" 
            class="flex items-center px-4 py-3 hover:bg-[#ffae00] hover:rounded-lg 
            {{ request()->routeIs('admin.users') ? 'bg-[#ffae00] text-black font-semibold rounded-lg' : '' }}">
        ...
        @foreach($dropdowns as $dropdown)
            ...
            <div x-data="{ open: {{ $isActive ? 'true' : 'false' }} }" class="mb-2">
                <button @click="open = !open"
                    class="flex items-center w-full px-4 py-3 text-white rounded-lg justify-between hover:bg-[#ffae00] transition
                    {{ $isActive ? 'bg-[#ffae00] text-black font-semibold' : '' }}">
                    <span class="flex items-center space-x-2">
                        <i class="{{ $dropdown['icon'] }} w-6"></i>
                        <span class="font-medium">{{ $dropdown['title'] }}</span>
                    </span>
                    <i :class="open ? 'fa-solid fa-chevron-up' : 'fa-solid fa-chevron-down'" class="transition-transform duration-300"></i>
                </button>
