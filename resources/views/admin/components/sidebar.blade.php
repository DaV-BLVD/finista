    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="h-16 flex items-center justify-center border-b border-gray-700 shadow-lg">
            <a href="{{ route('admin.dashboard') }}">
                <img src="{{ asset('images/logo.png') }}" alt="Logo" class="w-12 border-secondary rounded-md">
            </a>
        </div>
        ...
        <nav class="flex-1 px-2 py-4 space-y-2 overflow-y-auto styled-scrollbar">
            ...
            {{-- Dashboard --}}
            <a href="{{ route('admin.dashboard') }}" @click="sidebarOpen = false" class="flex items-center px-4 py-3 hover:bg-[#ffae00] hover:rounded-lg
            ...
            @if(Auth::user()->role === 'super_admin')
                <a href="{{ route('users.index') }}" @click="sidebarOpen = false"
                    class="flex items-center px-4 py-3 hover:bg-[#ffae00] hover:rounded-lg
                           {{ request()->routeIs('users.index') ? 'bg-[#ffae00] text-black font-semibold rounded-lg' : '' }}">
            ...
            @foreach($dropdowns as $dropdown)
                ...
                <div x-data="{ open: {{ $isActive ? 'true' : 'false' }} }" class="mb-2">
                    <button @click="open = !open" class="flex items-center w-full px-4 py-3 justify-between hover:bg-[#ffae00] transition
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-300"
                        ...
                        class="mt-1 overflow-hidden space-y-1 pl-6">
                        @foreach($dropdown['links'] as $link)
                            <a href="{{ route($link['route']) }}" @click="sidebarOpen = false"
                                class="flex items-center px-4 py-2 rounded-lg hover:bg-[#ffae00]
                                       {{ request()->routeIs(explode('.', $link['route'])[0] . '.*') ? 'bg-[#ffae00] text-black font-semibold' : 'text-white' }}">
            ...
            {{-- Footer Contacts --}}
            <a href="{{ route('footer_contacts.index') }}" @click="sidebarOpen = false"
                class="flex items-center px-4 py-3 hover:bg-[#ffae00] hover:rounded-lg
               {{ request()->routeIs('footer_contacts.*') ? 'bg-[#ffae00] text-black font-semibold rounded-lg' : '' }}">
